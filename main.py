...
def git_push_with_progress(commit_msg, extra_files=None, deleted_files=None, log_cb=None):
    """
    Lakukan git add → commit → push dengan progress callback.
    log_cb(pct, message) — dipanggil dari thread.
    extra_files: list path file yg akan di-add (selain list_map.json).
    deleted_files: list path file yg akan di-remove (git rm).
    """
    def log(pct, msg):
        if log_cb:
            log_cb(pct, msg)
    ...
    # Step 1: git add list_map.json + extra files
    files_to_add = [JSON_FILE]
    if extra_files:
        files_to_add += extra_files

    # Step 1b: git rm untuk deleted files
    if deleted_files:
        log(10, f"🗑️  Menghapus file dari repo: {', '.join(deleted_files)}")
        for df in deleted_files:
            out, err, code = git_cmd(["rm", df])
            if code != 0 and "did not match any files" not in err:
                log(-1, f"❌ Error git rm {df}: {err}")
                return False, err

    log(15, f"📁 git add {', '.join(files_to_add)}")
    ...
    # Step 2: git commit
    log(50, f"💾 git commit -m \"{commit_msg}\"")
...
class RullzsyHUB(tk.Tk):
    ...
    def _build_ui(self):
        ...
        apply_btn = HoverButton(
            ...
            hover_bg=["accent2"], hover_fg=C["white"],
    ...
    def _delete_map(self, idx, m):
        ...
        if ok:
            # PERBAIKAN: Hapus file .json dari repo saat delete map
            ...
            if fname and os.path.exists(fpath):
                try:
                    ...
                    messagebox.showerror("Error", f"Gagal hapus file: {ex}")
                    return
            
            self.data["maps"].pop(idx)
            save_data(self.data)
            
            # Jika ada file yg dihapus, push ke git
            if deleted_files:
                msg = f"Delete map: {m['name']}"
                def task():
                    git_push_with_progress(msg, deleted_files=deleted_files)
                threading.Thread(target=task, daemon=True).start()
            
            self._set_status(f"'{m['name']}' dihapus.", C["danger"])
            self._refresh()
